 ...
 * base: 0x00;
 * limit: 0xfffff
 * (actual limit will be 0xffffff because granularity bit set to 1, so page size 4KiB);
 * access: 0x92
 * 1 (present) 00 (privilege) 1 (empty/reserved) 1 (executable)  0 (conforming) 1 (readable) 0 (accessed)
 * flags: 0xc
 ...
 * (actual limit will be 0xffffff because granularity bit set to 1, so page size 4KiB);
 * access: 0x92
 * 1 (present) 00 (privilege) 1 (empty/reserved) 0 (executable)  0 (direction - segment grows up) 1 (writeable) 0 (accessed)
 * flags: 0xc
 * 1 (granularity) 1 (size) 00 (empty/reserved)
 */
